...
const Island = ({isRotating, setisRotating, setCurrentStage, ...props}) => {
  ...
  const keyPressed = useRef({ left: false, right: false });
  const keyboardRotationSpeed = useRef(0);
  const maxKeyboardSpeed = 0.02;
  const keyboardAcceleration = 0.001;
  ...
  const handleWheel = useCallback((e) => {
    ...
    const scrollDelta = e.deltaY * 0.00005; // Adjust sensitivity
    
    // Add to existing rotation speed instead of replacing it
    rotationSpeed.current += scrollDelta;
    
    // Clamp the speed to prevent it from getting too fast
    const maxScrollSpeed = 0.1;
    rotationSpeed.current = Math.max(-maxScrollSpeed, Math.min(maxScrollSpeed, rotationSpeed.current));
  }, []);

  const handleKeyDown = useCallback((e) => {
    if (e.key === 'ArrowLeft') {
      keyPressed.current.left = true;
      if (!isRotating) setisRotating(true);
    } else if (e.key === 'ArrowRight') {
      keyPressed.current.right = true;
      if (!isRotating) setisRotating(true);
    }
  ...
  const handleKeyUp = useCallback((e) => {
    if (e.key === 'ArrowLeft') {
      keyPressed.current.left = false;
    } else if (e.key === 'ArrowRight') {
      keyPressed.current.right = false;
    }
    
    // Stop rotating only if no keys are pressed
    if (!keyPressed.current.left && !keyPressed.current.right) {
      setisRotating(false);
    }
  }, [setisRotating]);
  ...
  }, [gl, handlePointerDown, handlePointerUp, handlePointerMove, handleWheel, handleKeyDown, handleKeyUp]);
  
  useFrame(() => {
    // Handle keyboard rotation with acceleration and smooth deceleration
    if (keyPressed.current.left || keyPressed.current.right) {
      const direction = keyPressed.current.left ? 1 : -1;
      
      // Accelerate towards max speed
      keyboardRotationSpeed.current += keyboardAcceleration * direction;
      keyboardRotationSpeed.current = Math.max(
        -maxKeyboardSpeed, 
        Math.min(maxKeyboardSpeed, keyboardRotationSpeed.current)
      );
      
      islandRef.current.rotation.y += keyboardRotationSpeed.current;
    } else {
      // Apply damping to keyboard rotation when no keys are pressed
      keyboardRotationSpeed.current *= keyboardDampingFactor;
      if (Math.abs(keyboardRotationSpeed.current) < 0.0001) {
    ...
    // Handle mouse/touch rotation inertia
    if (!isRotating && !keyPressed.current.left && !keyPressed.current.right) {

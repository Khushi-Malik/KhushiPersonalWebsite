...
const Island = ({isRotating, setisRotating, setCurrentStage, ...props}) => {
  ...
  const rotationSpeed = useRef(0); // Adjust rotation speed as needed
  const dampingFactor = 0.95; // Damping factor to slow down rotation
  ...
  const handlePointerMove = useCallback((e) => {
    ...
    if (isRotating) {
        ...
        islandRef.current.rotation.y += delta * 0.01 * Math.PI; // Adjust sensitivity as needed

        lastX.current = clientX;
        rotationSpeed.current = delta * 0.01 * Math.PI; // Store the speed for inertia // Update rotation on move
    }
  ...
  useFrame(() => {
    if (!isRotating) {
      ...
      if (Math.abs(rotationSpeed.current) < 0.001) {
        rotationSpeed.current = 0; // Stop if speed is very low
      }

      islandRef.current.rotation.y += rotationSpeed.current; // Continue rotating with inertia
